...
export default definePlugin({
    ...
    handleUpdateAttachments(newMessage: MLMessage): MLAttachment[] {
        const oldMessage = MessageStore.getMessage(
            newMessage.channel_id,
            newMessage.id
        ) as MLMessage | undefined;
        ...
        const newMap = new Map(
            newAttachments.map(a => [a.id, a])
        );
        const result: MLAttachment[] = [];
        for (const oldAttachment of oldAttachments) {
            const updated = newMap.get(oldAttachment.id);
            if (updated) {
                result.push({
                    ...oldAttachment,
                    ...updated,
                ...
                newMap.delete(oldAttachment.id);
            } else {
                result.push({
                    ...oldAttachment,
                    deleted: true
        ...
        for (const attachment of newMap.values()) {
            result.push({
                ...attachment,
                deleted: false
            });
